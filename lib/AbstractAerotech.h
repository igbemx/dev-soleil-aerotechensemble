...
public:
	...
	**/
	...
	/**
	* 	set the mode of wait of a task
	*	NOWAIT :  No wait (continue program execution)
	*	MOVEDONE :  Wait for motion to be completed (default)
	*	INPOS :  Wait for motion to be completed and axis is in position 
	*
	**/
	bool set_wait_mode (const std::string& wait_mode);
	...
	**/
	...
	virtual bool axis_calibration_is_active (void)=0;
	...
	**/
	...
	/**
	*  The function "set_axis_default_speed" returns the velocity target as a double.
